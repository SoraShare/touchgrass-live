...
# Add Kronos root directory to path if available
KRONOS_ROOT = Path(__file__).resolve().parents[3] / "Kronos"
...
class KronosBridge:
    ...
    def predict_trend(self, symbol: str, historical_prices: Optional[List[float]] = None) -> Dict[str, Any]:
        ...
        if not self.enabled or not historical_prices or len(historical_prices) < 5:
            # Fallback baseline momentum calculation if model not loaded or data limited
